...
static void *migration_thread(void *opaque)
{
...
#ifdef SMEMV
			initial_bytes = qemu_ftell(s->to_dst_file) + subhost_bytes;
#else
    ...
    if (s->state == MIGRATION_STATUS_COMPLETED) {
#ifdef SMEMV
		uint64_t transferred_bytes = qemu_ftell(s->to_dst_file) + subhost_bytes;
#else
        uint64_t transferred_bytes = qemu_ftell(s->to_dst_file);
#endif
